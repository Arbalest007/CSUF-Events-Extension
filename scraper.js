...
async function start() {
  ...
  let eventlist = driver.findElements(
    By.xpath(
      '//ul[@class="EventContainer_UL"]//div[@class="EventCard_Shell Events_CalendarListDetails "]'
    )
  );
  ...
  for (let event of await eventlist) {
    ...
    try {
      let destination = await page
        .findElement(
          By.xpath(
            '//*[@id="EventFullFrame"]/div/div[2]/div[2]/div[2]/div/p[1]'
          )
        )
        .getText();
      let info = {
        location: destination,
        event_title: await event
          .findElement(By.className("EventTitle"))
          .getText(),
        event_time: await event
          .findElement(By.className("EventTime"))
          .getText(),
        event_month: await driver
          .findElement(By.xpath('//p[@class="EventFull_month"]'))
          .getText(),
        event_day: await driver
          .findElement(By.xpath('//p[@class="EventFull_day"]'))
          .getText(),
        link: await page
          .findElement(
            By.xpath(
              '//*[@id="EventFullFrame"]/div/div[2]/div[2]/div[2]/div/p[2]'
            )
          )
          .getText(),
      };
